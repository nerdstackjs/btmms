...
func (reservation *EdReservation) Update(id string) map[string]interface{} {
	...
	db.Model(&reservation).Where("res_uuid = ?", id).Updates(EdReservation{ReservationStatus: reservation.ReservationStatus, CancellationReason: reservation.CancellationReason, BusId: reservation.BusId, Route: reservation.Route})
